...
class PushPull_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qe = self.qe_lif(q_conv_out)
        qi = self.qi_lif(-q_conv_out)

        qe = qe.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N)
        qi = qi.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N)
...
class LiDiff_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.mul(filtered_q, k) # Shape: [T, B, num_heads, head_dim, N]

        del k_conv_out, k_bn_out, k, filtered_q
...
class LiDiffTransformer(nn.Module):
    ...
    def forward(self, x):
        # x = x + checkpoint.checkpoint(self.tssa, x, use_reentrant=False)
        x = x + self.tssa(x)
        x = x + self.mlp(x)
...
class SpikingTransformer(nn.Module):
    ...
    def forward(self, x):
        x = x + checkpoint.checkpoint(self.ssa, x, use_reentrant=False)
        # x = x + self.ssa(x)
        x = x + self.mlp(x)
