...
__all__ = ['QKFormer', 'SpikingTransformer', 'PPTransformer', 'LiDiffTransformer', 'TokenSpikingTransformer', 'QKFormer_10_768']
...
class LiDiff_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qe_raw, qi_raw = q_bn_out.chunk(2, dim=1) # Shape: [T*B, C/2, N] each

        # Reshape back to [T, B, C/2, N]
        qe_raw = qe_raw.reshape(T, B, C // 2, N)
        qi_raw = qi_raw.reshape(T, B, C // 2, N)

        # Apply separate LIFs to qe and qi
        qe = self.qe_lif(qe_raw)
        qi = self.qi_lif(qi_raw)

        # Reshape for multi-head attention
        # Shape: [T, B, num_heads, q_head_dim, N]
        qe = qe.reshape(T, B, self.num_heads, self.q_head_dim, N)
        qi = qi.reshape(T, B, self.num_heads, self.q_head_dim, N)

        # --- Key Processing ---
        k_conv_out = self.k_conv(x_for_qkv) # Shape: [T*B, C, N]
        k_bn_out = self.k_bn(k_conv_out)   # Shape: [T*B, C, N]
        k_bn_out = k_bn_out.reshape(T, B, C, N) # Shape: [T, B, C, N]
        k = self.k_lif(k_bn_out) # Apply LIF. Shape: [T, B, C, N]
        ...
        filtered_q = self.merge_lif(qe_sum - qi_sum) # Shape: [T, B, num_heads, 1, N]

        # Multiply filtered_q (attention driver) with k (value)
        # Broadcasting filtered_q over head_dim of k
        x_attn = torch.mul(filtered_q, k) # Shape: [T, B, num_heads, head_dim, N]

        # Apply the final attention LIF *after* the multiplication
        x_attn_spikes = self.attn_lif(x_attn)

        # --- Projection ---
        # Flatten heads: [T, B, C, N]
        x_out = x_attn_spikes.reshape(T, B, C, N)

        # Apply projection layers
        x_out_flat = x_out.flatten(0, 1) # Shape: [T*B, C, N]
        proj_out = self.proj_conv(x_out_flat) # Shape: [T*B, C, N]
...
class spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, pretrained_cfg=None, transformer_classes = None
                 ):
        super().__init__()
