...
def main():
    ...
    model = create_model(
        ...
        T=4, transformer_classes=transformer_classes
   )
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            is_best = eval_metrics['top1'] > best_acc1
            best_acc1 = max(eval_metrics['top1'], best_acc1)
            
            if args.local_rank==0:
                checkpoint_dir = wandb.run.dir
                state = build_checkpoint_state(model, optimizer, epoch, loss_scaler, model_ema, args)
                save_checkpoint(state, is_best, checkpoint_dir)
                if is_best:
                    print(f'Saving model at epoch {epoch} with val acc. of {best_acc1}')            
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    if args.local_rank == 0:
        log_data = {'test/acc1': top1_m.avg,
                    'test/acc5': top5_m.avg,
                    'test/loss': losses_m.avg
                    }
        wandb.log(log_data, step=args.current_epoch)
